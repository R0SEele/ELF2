...
class EnvironmentMonitor:
	def __init__(self, sensors_cfg):
		...
		self._sht30_enabled = _is_enabled(sht_cfg)
		...
		self._scd40_enabled = _is_enabled(scd_cfg)
		...
		self._mq135_enabled = _is_enabled(mq_cfg)
		if self._mq135_enabled:
			try:
				self._mq135 = MQ135(
					adc=MQ135ADC(
	...
	def read_all(self):
		...
		if not self._sht30_enabled:
			readings["sht30"] = {"ok": None, "disabled": True}
		elif self._sht30 is None:
		...
		if not self._scd40_enabled:
			readings["scd40"] = {"ok": None, "disabled": True}
		elif self._scd40 is None:
		...
		if not self._gy302_enabled:
			readings["gy302"] = {"ok": None, "disabled": True}
		elif self._gy302 is None:
		...
		if not self._mq135_enabled:
			readings["mq135"] = {"ok": None, "disabled": True}
		elif self._mq135 is None:
