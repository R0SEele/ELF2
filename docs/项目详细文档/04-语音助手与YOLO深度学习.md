...
- **`.rknn` 模型**（RK3588、INT8 `i8`）：`mango_yolo11_rk3588_i8.rknn`（≈4.21MB，生产芒果模型，YAML 默认）、`fruits1_yolo11_rk3588_i8.rknn`（通用水果）、demo 目录实际存在 `rknnModel/best.rknn`。注意：`deeplearning/demo/main_camera_fps_v8.py` 当前仍写死引用 `rknnModel/fruits1.rknn`，运行该参考 demo 前需改为 `best.rknn` 或补齐对应模型文件。
...
- 摄像头源 `/dev/video21`。
- `camera.yaml`：旧预览/采集配置保留项，当前生产检测与 Qt 视频流不读取该文件。
